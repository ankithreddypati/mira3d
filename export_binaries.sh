...
if command -v colmap &>/dev/null; then
    ...
    echo "⚠️  COLMAP not found, skipping..."
fi
...
if command -v ffmpeg &>/dev/null; then
    ...
    echo "⚠️  FFmpeg not found, skipping..."
fi
...
if command -v convert &>/dev/null; then
    ...
    echo "⚠️  convert not found, skipping..."
fi
...
if [ -x "$BRUSH_BIN" ]; then
    ...
    echo "⚠️  Brush binary not found, skipping..."
fi

########################################
# Vulkan Mesa fallback drivers
########################################
echo "📦 Exporting Mesa Vulkan drivers..."
for drv in /lib/x86_64-linux-gnu/libvulkan_*.so*; do
    [ -e "$drv" ] && cp -v "$drv" export/vulkan/lib/
done
...
if [[ -n "$NVIDIA_SO" && -f "$NVIDIA_SO" ]]; then
    ...
    echo "📦 Writing ICD JSON for $ICD_NAME"
...
EOF
    ...
    echo "❌ NVIDIA libGLX_nvidia.so.0 not found — GPU will not be usable."
fi

########################################
# Vulkan loader (libvulkan.so.1)
########################################
LOADER=$(ldconfig -p | grep 'libvulkan.so.1' | head -n1 | awk '{print $NF}')
if [ -n "$LOADER" ] && [ -f "$LOADER" ]; then
    echo "📦 Copying Vulkan loader: $LOADER"
    cp -v "$LOADER" export/vulkan/lib/
else
    echo "⚠️  Vulkan loader libvulkan.so.1 not found."
fi
...
if command -v vulkaninfo &>/dev/null; then
    ...
    echo "⚠️  vulkaninfo not found, skipping..."
fi

echo "✅ Export complete! Artifacts in ./export/"